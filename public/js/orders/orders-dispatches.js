...
/***/ "./resources/js/pages/orders/orders-dispatches.js":
/*!********************************************************!*\
  !*** ./resources/js/pages/orders/orders-dispatches.js ***!
  \********************************************************/
/*! no exports provided */
...
$(function () {
  $(document).on('click', '.show-dispatch', function (e) {
    var t = $(e.currentTarget);
    _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
      method: 'get',
      url: "/orders/".concat($('body #order-id').val(), "/dispatches/").concat(t.data('id'), "/show"),
      data: {}
    }), function (res) {
      $.confirm({
        ...
        buttons: {
          confirm: {
            ...
            action: function action() {
              window.open("/orders/".concat($('body #order-id').val(), "/dispatches/").concat($('body #dispatch_id').val(), "/print"));
            }
          }
        },
        onClose: function onClose() {},
        onContentReady: function onContentReady() {
          this.$content.find('#quantity').mask("#.##0,00", {
            reverse: true
          });
        }
      });
  ...
  $(document).on('click', '.delete-product-from-dispatch', function (e) {
    var t = $(e.currentTarget);
    var form = t.parents('.form-row');
    form.remove();
  });
  $(document).on('click', '#add-product-to-dispatch', function (e) {
    var t = $(e.currentTarget);
    var form = t.parents('.form-row');
    var productID = form.find('#product-id').val();
    form.find('#product-id').val(-1);
    var quantity = form.find('#quantity').val();
    form.find('#quantity').val('');
    var productQnt = Math.random().toString(36).substring(7);
    var fieldsHtml = $(document).find('#form-example').clone();
    fieldsHtml.removeAttr('id');
    fieldsHtml.removeClass('hide');
    fieldsHtml.addClass('order-dispatch-products-form');
    var productSelect = fieldsHtml.find('select');
    productSelect.val(productID);
    productSelect.attr('name', "products[".concat(productQnt, "][product_id]"));
    productSelect.attr('readonly', 'readonly');
    var quantityInput = fieldsHtml.find('input');
    quantityInput.val(quantity);
    quantityInput.attr('name', "products[".concat(productQnt, "][quantity]"));
    quantityInput.attr('readonly', 'readonly');
    $('body #order-dispatch-form').append(fieldsHtml);
  });
  $('#dispatches-list').DataTable({
    "scrollX": true,
    columnDefs: [{
      orderable: false,
      searchable: false,
  ...
  $('body').on('click', '.delete-dispatch', function (e) {
    var t = $(e.currentTarget);
    _utils_alert__WEBPACK_IMPORTED_MODULE_1__["deleteConfirm"](null, function () {
      _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
        method: 'delete',
        url: '/orders/dispatches/' + t.data('id'),
        data: {}
      }), function () {
        _utils_alert__WEBPACK_IMPORTED_MODULE_1__["successReload"]('Envio removido.');
      });
    });
  });
  $('#new-order-dispatch').on('click', function () {
    _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
      method: 'get',
      url: "/orders/".concat($('body #order-id').val(), "/dispatches/create"),
      data: {}
    }), function (res) {
      $.confirm({
        ...
        buttons: {
          confirm: {
            ...
            btnClass: 'btn-green',
            action: function action() {
              var form = this.$content.find('#order-dispatch-form');
              var formData = form.serializeControls();
              formData.order_id = $('body #order-id').val();
              $.each(formData.products, function (k, v) {
                formData.products[k].quantity = v.quantity.replace('.', '').replace(',', '.').trim();
              });
              _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
                method: 'post',
                ...
                _utils_alert__WEBPACK_IMPORTED_MODULE_1__["successReload"]('Envio realizado com sucesso.');
              });
            }
          }
        },
...
/***/ 17:
/*!**************************************************************!*\
  !*** multi ./resources/js/pages/orders/orders-dispatches.js ***!
  \**************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/order-management/resources/js/pages/orders/orders-dispatches.js */"./resources/js/pages/orders/orders-dispatches.js");

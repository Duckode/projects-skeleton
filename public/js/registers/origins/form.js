...
/***/ "./resources/js/pages/registers/origins/form.js":
/*!******************************************************!*\
  !*** ./resources/js/pages/registers/origins/form.js ***!
  \******************************************************/
/*! no exports provided */
...
$(function () {
  $('body').find('#zipcode').mask('00.000-000');
  $('body').on('blur', '#zipcode', function (e) {
    ...
    if (zipcode != '') {
      _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
        ...
        var data = res.data;
        $('body #state-id').val(data.state_id);
        loadCitiesByState(data.state_id, data.city_id);
        $('body #district').val(data.district);
        $('body #street').val(data.street);
      });
    }
  });
  $('body').on('change', '#state-id', function (e) {
    var t = $(e.currentTarget);
    var id = t.val();
  ...
  $('body #save').on('click', function () {
    var form = _utils_request__WEBPACK_IMPORTED_MODULE_0__["getFormData"]($('#origin-form'));
    form.address = _utils_request__WEBPACK_IMPORTED_MODULE_0__["getFormData"]($('body #address-form'));
    form.address.zipcode = form.address.zipcode.replace(/([.-])+/g, '');
    delete form.address.addressable_id;
    delete form.address.addressable_type;
    var requestData;

    if ($('#origin-id').val() != undefined) {
      requestData = {
        method: 'put',
        url: '/registers/origins/' + $('#origin-id').val(),
        message: 'Dados da origem atualizados.'
      };
    } else {
      requestData = {
        method: 'post',
        url: '/registers/origins',
        message: 'Dados da origem inseridos.'
      };
    }
    ...
    _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
      ...
      _utils_alert__WEBPACK_IMPORTED_MODULE_1__["successReload"](requestData.message, function () {
        return location.href = '/registers/origins/';
      });
    });
  });
});
...
function loadCitiesByState(stateId) {
  ...
  _utils_request__WEBPACK_IMPORTED_MODULE_0__["handle"](axios({
    ...
    $.each(res.data, function (key, value) {
      $('body #city-id').append('<option value=' + value.id + '>' + value.name + '</option>');

      if (cityId) {
        $('body #city-id').val(cityId);
      }
    });
  });
}
...
/***/ 9:
/*!************************************************************!*\
  !*** multi ./resources/js/pages/registers/origins/form.js ***!
  \************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /var/www/html/order-management/resources/js/pages/registers/origins/form.js */"./resources/js/pages/registers/origins/form.js");
